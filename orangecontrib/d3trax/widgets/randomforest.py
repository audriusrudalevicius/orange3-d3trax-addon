...
class OWRandomForest(OWBaseLearner):
    ...
    weight_index = settings.Setting(0)
    ...
    def add_classification_layout(self, box):
        self.n_estimators_spin = gui.comboBox(
            box, self, "weight_index", orientation=Qt.Horizontal,
            label="Weights:", items=[i for i in self.weight_lbl],
            callback=self.settings_changed)
        pass
    ...
    def get_learner_parameters(self):
        ...
        return (
            ("Class wight", self.weight_lbl[self.weight_index]),
